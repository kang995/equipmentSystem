...
enum Api {
  ...
  USER_ADD_GROUP = '/system/group/addUserGroup/', //用户添加到用户组
  USER_MODAL = '/system/group/getUserGroupCheck/', //获取添加用户组时用户组列表
...
//添加到用户组
export const getAddGroupApi = (params, userId: string) =>
  defHttp.post({
    url: Api.USER_ADD_GROUP + userId,
    params,
  });
//修改用户基本信息
...
//获取添加用户组时用户组列表
export const getModalListApi = (userId: string) =>
  defHttp.post({
    url: Api.USER_MODAL + userId,
  });
