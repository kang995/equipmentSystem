      ...
      <a-col :span="24">
        <a-card>
          ...
          <div v-if="specialWorkCount.length > 0">
            <LineBar :data="specialWorkCount" :title="'检修工单'" />
          </div>
          <Empty v-else />
        </a-card>
      </a-col>
...
<script setup lang="ts">
  ...
  import { getOverhaulStatsApi } from '/@/api/statisticalAnalysis/WorkOrder';
  ...
  const workCount = ref<any>([{ title: '检修工单数量' }]);
  ...
  function getWorkCount(val?) {
    getOverhaulStatsApi({ timeType: val }).then((res) => {
