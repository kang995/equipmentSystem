<template>
  <PageWrapper contentBackground>
    ...
    <template v-if="!partFlag && (activeKey === '1' || activeKey === '2')">
      <div :class="[!partFlag && activeKey === '1' ? 'ml-[26%] mt-[-32px] mb-[12px]' : 'm-[12px]']">
...
<script setup lang="ts">
  ...
  import relevantFailure from './relevantFailure.vue';
  import { UpkeepWorkOrderDealResultApi } from '/@/api/device-service/service';
  ...
  async function handleSubmit() {
    ...
    console.log('数据', objs);
    UpkeepWorkOrderDealResultApi(objs).then(() => {
